...
def read_conll_file(fname):
    ...
    for line in open(fname, 'r'):
        ...
        if len(split_line) > 1:
            if (
                split_line[0] != 'doc_id' and line[0] != '#'
            ):  # Skip header and/or document tags
                conll_list.append(split_line)
        if (
            not line.strip() or line == '#end document'
        ):  # Empty line equals new sentence
            num_sentences += 1
...
def make_mention(begin, end, tree, mention_type, sentNum, ngdata):
    ...
    if mention_type.lower()[:2] == 'np':
        if mention_type.lower() == 'np_comma':
            ...
            for i in range(len(new_ment.tokenAttribs)):
                if (
                    'rel' in new_ment.tokenAttribs[i]
                    and new_ment.tokenAttribs[i]['rel'] == 'hd'
                ):
                    headRange.append(i)
            if len(headRange) == 0:
            ...
            new_ment.head_begin = (
                int(head_node.attrib['begin']) - new_ment.begin
            )
        ...
        mention_type.lower() == 'name' or mention_type.lower()[:3] == 'mwu'
    ):  # Add last part of names as headword
        new_ment.head_begin = len(new_ment.tokenList) - 1
        new_ment.head_end = len(new_ment.tokenList)
        new_ment.headWords = new_ment.tokenList[-1:]
        if not re.search(
            '[a-zA-Z]', new_ment.tokenList[-1]
        ):  # Head cannot just be numbers, need to contain letters
    ...
    new_ment = add_mention_features(
        new_ment, ngdata
    )  # Add features for pronoun resolution
    return new_ment
...
def add_mention_features(mention, ngdata):
    ...
    try:
        ...
        if (
            'num' not in attribs
            and 'rnum' not in attribs
            and 'getal' in attribs
        ):
    ...
    if 'neclass' in attribs or mention.type.lower() == 'name':
        ...
        try:
            gender_data = [
                a + b
                for a, b in zip(
                    gender_data, ngdata[' '.join(lowered_token_list)]
                )
            ]
        except KeyError:
                ...
                try:
                    for lowered_head in lowered_head_list:
                        gender_data = [
                            a + b
                            for a, b in zip(
                                gender_data, ngdata[lowered_head + ' !']
                            )
                        ]
                        gender_data = [
                            a + b
        ...
        if gender_data[0] > sum(gender_data) / 3:
            ...
            gender_data[0] > sum(gender_data) / 3
            and gender_data[1] > sum(gender_data) / 3
        ):
            mention.gender = 'nonneuter'
...
def sort_mentions(mention_id_list, mention_dict):
    return sorted(
        mention_id_list,
        key=lambda x: (
            mention_dict[x].sentNum,
            mention_dict[x].begin,
            mention_dict[x].end,
        ),
    )
...
def add_mention(mention_list, new_mention):
    for old_mention in mention_list:
        if (
            old_mention.begin == new_mention.begin
            and old_mention.end == new_mention.end
            and old_mention.sentNum == new_mention.sentNum
        ):
            return mention_list
...
def initialize_clusters(mention_dict, mention_id_list):
    ...
    for mention_id in mention_id_list:
        ...
        new_cluster = Cluster(
            mention.ID
        )  # Initialize with same ID as initial mention
...
def print_mentions_inline(sentenceDict, mention_id_list, mention_dict):
    for sentNum in sentenceDict:
        ...
        closingBrackets = (
            ''
        )  # Print closing brackets for mention that close at end of sentence
    ...
    for sentNum in sentenceDict:
        ...
        for idx, token in enumerate(sentenceDict[sentNum].split(' ')):
            ...
            if gold_open >= resp_open:
                ...
                sys.stdout.write(
                    (resp_open - gold_open) * colour_text('[', 'red')
                )
            ...
            if gold_close >= resp_close:
                sys.stdout.write(
                    (gold_close - resp_close) * colour_text(']', 'yellow')
                )
...
def print_linked_mentions(
    ...
    for mention_id in mention_id_list:
        if (
            mention_dict[mention_id].clusterID
            != old_mention_dict[mention_id].clusterID
        ):
            linkings[old_mention_dict[mention_id].clusterID] = mention_dict[
                mention_id
            ].clusterID
    for sentNum in sentenceDict:
        sentLength = len(sentenceDict[sentNum].split(' '))
