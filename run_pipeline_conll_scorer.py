    ...
    for filename in os.listdir(dirname):
        if os.path.isfile(dirname + filename):
            if re.search('.xml.coref_ne$', filename) or re.search(
                ...
                processDocument(
                    dirname + filename, verbosity, sieveList, ngdata
                )
...
def postProcessScores(scores_dir, verbosity, onlyTotal=False):
    ...
    for filename in os.listdir(scores_dir):
        if os.path.isfile(scores_dir + '/' + filename) and re.search(
            '.scores$', filename
        ):
            docName = filename.split('_')[0]
            scores[docName] = process_conll_scorer_file(
                scores_dir + '/' + filename
            )
    ...
    try:
        coref_precision = (
            totals['blanc-special'][0] / totals['blanc-special'][2]
        )
    except ZeroDivisionError:
        coref_precision = 0
    ...
    with open(scores_dir + '/' + 'scores_overall', 'w') as out_file:
        ...
        for document in scores:
            ...
            try:
                doc_coref_precision = (
                    a['blanc-special'][0] / a['blanc-special'][2]
                )
            except ZeroDivisionError:
            ...
            try:
                doc_non_precision = (
                    a['blanc-special'][3] / a['blanc-special'][5]
                )
            except ZeroDivisionError:
                doc_non_precision = 0
            try:
                doc_coref_f = (
                    ...
                    * doc_coref_recall
                    * doc_coref_precision
                    / (doc_coref_recall + doc_coref_precision)
                )
            except ZeroDivisionError:
                doc_coref_f = 0
            try:
                doc_non_f = (
                    ...
                    * doc_non_recall
                    * non_precision
                    / (doc_non_recall + doc_non_precision)
                )
            except ZeroDivisionError:
                doc_non_f = 0
            ...
            scorestring = (
                '%s%05.2f/%05.2f/%05.2f\t%05.2f/%05.2f/%05.2f\t%05.2f/%05.2f/%05.2f\t%05.2f/%05.2f/%05.2f\t%05.2f/%05.2f/%05.2f\t%05.2f'
                % (
                    docName,
                    a['md'][2],
                    a['md'][5],
                    ...
                    a['blanc'][5],
                    a['blanc'][6],
                    doc_coref_recall * 100,
                    doc_coref_precision * 100,
                    doc_coref_f * 100,
                    doc_non_recall * 100,
                    doc_non_precision * 100,
                    doc_non_f * 100,
                    a['conll'][0],
                )
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'target',
        type=str,
        help='Path to a file or directory, in .conll format, for which to do coreference resolution.',
    )
    parser.add_argument(
        '-v',
        '--verbosity',
        type=str,
        help='Verbosity of output, can be either "high" or "low", default is "high"',
        default='high',
    )
    parser.add_argument(
        '-s',
