    ...
    for line in input_file:
        if line[0] != '#':
            if line.strip():
                output_file.write(
                    docName + '\t' + '0\t' * 4
                )  # Add in document name and dummy columns
            else:
                sentence_counter += 1
            if (
                sentence_counter == 6
            ):  # Officially 5 sentences, but first 6 because of  '9	06-Mar-08	_' kind of stuff
                break
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'input_file',
        type=str,
        help='Path to a file from the CLIN data to be processed for scoring.',
    )
