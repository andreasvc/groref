...
def processMarkable(parent_markable, id_list):
    if (
        'id' in parent_markable.attrib
    ):  # Check whether actual markable, or sentence
        mention_id = re.findall('[0-9]+', parent_markable.attrib['id'])[
            0
        ]  # Extract mention id
        if 'type' in parent_markable.attrib:  # Check whether it is coreferent
            if (
                parent_markable.attrib['type'] in coref_types
    ...
    if (
        parent_markable.find('markable') is not None
    ...
    for idx, token in enumerate(tokenArray):
        for idx2, mention_id in enumerate(token):
            if idx2 > 0:  # Skip first entry, which is the token itself
                ...
                if (
                    mention_id in coreferential_ids
                ):  # Resolve coreferential links, set all mentions that corefer to have same id, that of first mention of the coreference cluster
                    while mention_id in coreferential_ids:
                        original_mention_ids.append(
                            mention_id
                        )  # Keep this to check for closing bracket
                        mention_id = coreferential_ids[mention_id]
                    token[idx2] = mention_id
                if (
                    mention_id not in seenList
                ...
                if (
                    ...
                    if mention_id not in tokenArray[idx + 1][1:] and not set(
                        original_mention_ids
                    ).intersection(set(tokenArray[idx + 1][1:])):
                        token[idx2] += ')'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'input_dir',
        type=str,
        help='Path to a directory containing XML-files with COREA corpus data',
    )
    parser.add_argument(
        '--noInner',
        help='If this flag is given, corefIDs are only printed for first and last word of mention',
        dest='no_inner',
        action='store_true',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--printHeader',
        help='If this flag is provided, a header is printed with column names',
        dest='print_header',
        action='store_true',
    )
    ...
    for filename in filenames:
        if re.search('_inline.xml$', filename):
            ...
            conll_writer = csv.writer(
                conll_file, delimiter='\t', quoting=csv.QUOTE_NONE, quotechar=''
            )
            ...
            for sentence in root.iter('sentence'):
                ...
                try:
                    ...
                    paragraph_id = re.findall('p.[0-9]+', alpsent)[0][
                        2:
                    ]  # Extract paragraph number
                    sentence_id = re.findall('s.[0-9]+', alpsent)[0][
                        2:
                ...
                for token in tokenArray:
                    ...
                    if (
                        args.no_inner
                    ):  # Do not print cluster IDs except for at start and end of mention
                        new_IDs = []
                        for old_ID in token[1].split('|'):
                            if (
                                '(' in old_ID or ')' in old_ID
                            ):  # Check for brackets
