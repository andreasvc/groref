...
def sievePronounResolution(
    ...
    for cluster_id in cluster_id_list[:]:
        ...
        anaphor = mention_dict[
            cluster.mentionList[0]
        ...
        for sent_id in range(
            anaphor.sentNum, max(0, anaphor.sentNum - 3), -1
            ...
            if sent_id in mention_ids_per_sentence:  # Not empty
                for candidate_mention_id in mention_ids_per_sentence[sent_id]:
                    ...
                    for ment_id in candidate_cluster.mentionList:
                        ...
                        if (
                            cluster_mention.number == anaphor.number
                            or anaphor.number == 'unknown'
                        ):
                            matchNumber = True
                        if (
                            cluster_mention.gender == anaphor.gender
                            or anaphor.gender == 'unknown'
                        ):
                            matchGender = True
                        if anaphor.animacy == 'animate':
                            if (
                                cluster_mention.NEtype == 'person'
                                or cluster_mention.NEtype == 'misc'
                            ):
                                matchNER = True
                            if (
                                cluster_mention.NEtype == ''
                        ...
                        if cluster_mention.type.lower() == 'pronoun':
                            if (
                                cluster_mention.person == anaphor.person
