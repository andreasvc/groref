            ...
            if sent_id in mention_ids_per_sentence:  # Not empty
                for candidate_mention_id in mention_ids_per_sentence[sent_id]:
                    ...
                    for ment_id in candidate_cluster.mentionList:
                        ...
                        if appos:
                            for ids in mention_dict[ment_id].tokenAttribs:
                                for ana_ids in mention_dict[
                                    ...
                                    if (
                                        (ids['id'], ids['root']) in appos[0]
                                        and (ana_ids['id'], ana_ids['root'])
                                        in appos[1]
                                        and len(
                        ...
                        if pred:
                            for ids in mention_dict[ment_id].tokenAttribs:
                                for ana_ids in mention_dict[
                                    ...
                                    if (ids['id'], ids['root']) in pred[0] and (
                                        ana_ids['id'],
                                        ana_ids['root'],
                                    ) in pred[1]:
                                        madeLink = True
                        ...
                        if (
                            mention_dict[ment_id].type == 'name'
                    ...
                    for h in node:
                        ...
                        if 'mwu_root' in h.attrib:
                            ...
                            for node in h:
                                if (
                                    'rel' in node.attrib
                                    and node.attrib['rel'] == 'mwp'
                                ):
                                    ment.append(
                                        (node.attrib['id'], node.attrib['root'])
                                    )
